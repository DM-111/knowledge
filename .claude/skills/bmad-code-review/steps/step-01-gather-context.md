...
3. Construct `{diff_output}` from the chosen source.
   ...
   - Exclude `pnpm-lock.yaml` from `{diff_output}` by default. Treat pnpm lockfile churn as out of scope for CR unless the user explicitly asks to review lockfile or dependency-resolution changes.
   - If the only remaining diff content is `pnpm-lock.yaml`, HALT and tell the user there is nothing in CR scope unless they want to include the lockfile explicitly.
